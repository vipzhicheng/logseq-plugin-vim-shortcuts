...
✨ **完整的 Vim 模式编辑** - 支持普通、插入、可视（字符和行）和命令模式
🎯 **丰富的键盘绑定** - 50+ 可自定义快捷键
⚙️ **可视化设置界面** - 图形化界面轻松配置，智能默认值和工具提示
🔖 **标记系统** - 保存并快速跳转到常用页面和块
🎨 **块样式设置** - 背景色选择器
🔍 **智能搜索** - 支持智能大小写匹配的页内搜索
📋 **高级编辑** - 大小写转换、数字递增/递减、替换操作等
...
- `gu` - 将选中的块转换为小写
- `gU` - 将选中的块转换为大写
- `Mod+Shift+u` - 切换选中块的大小写
- `x` - 剪切选中的块
- `r` - 替换选中块中的第一个字符
- `Shift+V` - 退出可视行模式

> **注意**：可视行模式选择整个块，使得对多个块执行操作变得容易。
...
**方法 3**：为"打开设置"操作绑定自定义快捷键（在 Logseq 命令面板中搜索"Open Settings"以配置快捷键）
...
- ✅ **开关控制**：快速启用或禁用单个快捷键
- ✅ **验证功能**：自动格式验证和重复检测
- ✅ **智能重置按钮**："重置为默认"按钮仅在绑定与默认值不同时显示
- ✅ **默认值提示**：悬停在任何设置上可在工具提示中查看默认值
- ✅ **智能点击外部行为**：点击设置界面外部会关闭它，但点击设置、帮助、标记面板或 Element Plus 组件内部会保持打开状态
- ✅ **内置帮助**：通过帮助按钮访问完整文档

### 使用方法

1. **打开设置**：点击命令面板中的齿轮图标（⚙），或使用自定义快捷键（如已配置）
...
7. **重置为默认**：点击"重置为默认"（仅在与默认值不同时可见）恢复原始设置
8. **查看默认值**：悬停在任何设置上可在工具提示中查看其默认值
9. **保存更改**：点击"保存设置" - Logseq 会提示重启以使更改生效
...
- 🎁 [爱发电（中国用户）](https://afdian.com/a/vipzhicheng)
